...
@push('scripts')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // --- NİHAİ DÜZELTME: VERİLER ARTIK STATİK DOSYADAN DEĞİL, DOĞRUDAN CONTROLLER'DAN GELİYOR ---
...
<a href="/mesajlar/${station.user_id ?? ''}" class="popup-button flex-1 text-center text-xs">
    <i class="fas fa-comment-dots mr-1"></i>Mesaj
</a>
        ...
        // Su kaynaklarını veritabanından gelen dinamik veriyle yükle
        waterSources.forEach(source => {
            const marker = L.marker([source.latitude, source.longitude], {icon: waterIcon});
            waterLayer.addLayer(marker);
        });

        // Olayları veritabanından gelen dinamik veriyle yükle
